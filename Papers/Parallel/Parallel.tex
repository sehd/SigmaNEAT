...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        \Procedure{GetOutput}{$Individual$,$Input$}
            \State{$NEAT = CreatePhenotype(Individual.Genome)$}
            \For{$Node_i \in Substrate$}
                \For{$Node_j \not= Node_i \in Substrate$}
                    \State{$w_{ij}=NEAT.GetOutput(Node_i,Node_j)$}
                    \If{$w_{ij}<Threshold$}
                        \State{$w_{ij}=0$}
                    \EndIf
                \EndFor
        ...
        \Procedure{GetValueRecursive}{$w$,$Node$}
            ...
            \For{$ConnectedNode \in Substrate_{Node}$}
                ...
                \State{$sum \gets sum + value * w_{ij}$}
            \EndFor
            \State{$Node.Value \gets ApplyActivation(sum)$}
            \State \Return {$Node.Value$}
        \EndProcedure
    \end{algorithmic}
\end{algorithm}
